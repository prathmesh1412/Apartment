            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-2 bg-gray-50 p-1 rounded-xl border border-gray-205">
                <button
                  ...
                  className={`py-2 px-3 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all cursor-pointer ${activeMode === 'building'
                ...
                <button
                  ...
                  className={`py-2 px-3 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all cursor-pointer ${activeMode === 'wing'
                    ? 'bg-blue-600 text-white shadow-sm'
                    : 'text-gray-500 hover:bg-gray-150/70 hover:text-gray-700'
                    }`}
                >
                ...
                <button
                  ...
                  className={`py-2 px-3 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all cursor-pointer ${activeMode === 'flat'
                    ? 'bg-blue-600 text-white shadow-sm'
                    : 'text-gray-500 hover:bg-gray-150/70 hover:text-gray-700'
                    }`}
                >
...
export default function ComparisonTable({
  ...
  // Resizable & Minimizable panel state
  ...
  const [minimizedPanels, setMinimizedPanels] = useState<{ left: boolean; middle: boolean; right: boolean }>({
    left: false,
    middle: false,
    right: false
  });

  const toggleMinimizePanel = (panel: 'left' | 'middle' | 'right') => {
    setMinimizedPanels(prev => {
      const nextState = { ...prev, [panel]: !prev[panel] };
  ...
  const recalculatePanelWidths = (minState: { left: boolean; middle: boolean; right: boolean }) => {
    const activeKeys = (['left', 'middle', 'right'] as const).filter(k => !minState[k]);
    if (activeKeys.length === 0) {
      setMinimizedPanels({ left: false, middle: false, right: false });
      setPanelWidths({ left: DEFAULT_LEFT, middle: DEFAULT_MIDDLE, right: DEFAULT_RIGHT });
      return;
    }
    const equalWidth = 100 / activeKeys.length;
    setPanelWidths({
      left: minState.left ? 0 : equalWidth,
      middle: minState.middle ? 0 : equalWidth,
      right: minState.right ? 0 : equalWidth
    });
  };
  ...
  useEffect(() => {
    ...
    const handleRestoreEvent = () => restoreAllPanels();
    window.addEventListener('restore-all-tables', handleRestoreEvent);

    return () => {
      window.removeEventListener('mousemove', handleMouseMove);
      window.removeEventListener('mouseup', handleMouseUp);
      window.removeEventListener('restore-all-tables', handleRestoreEvent);
    };
      ...
      {activeTab === 'apply-oc' ? (
        ...
        <div className="w-full flex flex-col bg-white p-4">
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse text-[10.5px] border border-gray-300">
              <thead>
                <tr className="bg-[#eef2ff] border-b-2 border-blue-300 text-[#002fbe] font-black uppercase tracking-wider h-[36px]">
                  <th className="py-2 px-3 text-[#002fbe] font-black border border-gray-300">Wing ID</th>
                  <th className="py-2 px-3 text-[#002fbe] font-black border border-gray-300">Wing Name</th>
                  <th className="py-2 px-3 text-center text-[#002fbe] font-black border border-gray-300">Floors</th>
                  <th className="py-2 px-3 text-center text-[#002fbe] font-black border border-gray-300">Total Units</th>
                  <th className="py-2 px-3 text-right text-[#002fbe] font-black border border-gray-300">Total Surveyed Area</th>
                  <th className="py-2 px-3 text-right text-blue-900 bg-blue-100/60 font-black border border-blue-300">Total Rateable Value</th>
                  <th className="py-2 px-3 text-right text-amber-900 bg-amber-100/60 font-black border border-amber-300">Old Demand</th>
                  <th className="py-2 px-3 text-right text-indigo-900 bg-indigo-100/70 font-black border border-indigo-300">New Demand</th>
                  <th className="py-2 px-3 text-right text-emerald-900 bg-emerald-100/70 font-black border border-emerald-300">Estimated Annual Tax</th>
                </tr>
              </thead>
              <tbody className="text-slate-700 font-semibold">
                <tr className="h-[36px]">
                  <td className="py-1.5 px-3 font-black text-[#006a4e] border border-gray-200">A Wing</td>
                  <td className="py-1.5 px-3 text-red-600 font-bold border border-gray-200">Krishna Block</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">G + 7</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">19</td>
                  <td className="py-1.5 px-3 text-right border border-gray-200">12,456 sqft</td>
                  <td className="py-1.5 px-3 text-right font-black text-blue-700 border border-gray-200">₹4.39L</td>
                  <td className="py-1.5 px-3 text-right font-extrabold text-slate-700 border border-gray-200">₹43,920</td>
                  <td className="py-1.5 px-3 text-right font-black text-[#002fbe] border border-gray-200">₹68,850</td>
                  <td className="py-1.5 px-3 text-right text-green-600 font-bold border border-gray-200">₹68,850</td>
                </tr>
                <tr className="h-[36px] bg-slate-50/30">
                  <td className="py-1.5 px-3 font-black text-purple-700 border border-gray-200">B Wing</td>
                  <td className="py-1.5 px-3 text-red-600 font-bold border border-gray-200">Sai Block</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">G + 7</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">19</td>
                  <td className="py-1.5 px-3 text-right border border-gray-200">11,920 sqft</td>
                  <td className="py-1.5 px-3 text-right font-black text-blue-700 border border-gray-200">₹4.12L</td>
                  <td className="py-1.5 px-3 text-right font-extrabold text-slate-700 border border-gray-200">₹43,920</td>
                  <td className="py-1.5 px-3 text-right font-black text-[#002fbe] border border-gray-200">₹64,240</td>
                  <td className="py-1.5 px-3 text-right text-green-600 font-bold border border-gray-200">₹64,240</td>
                </tr>
                <tr className="h-[36px]">
                  <td className="py-1.5 px-3 font-black text-green-700 border border-gray-200">C Wing</td>
                  <td className="py-1.5 px-3 text-red-600 font-bold border border-gray-200">Ganesh Block</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">G + 7</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">15</td>
                  <td className="py-1.5 px-3 text-right border border-gray-200">9,850 sqft</td>
                  <td className="py-1.5 px-3 text-right font-black text-blue-700 border border-gray-200">₹3.22L</td>
                  <td className="py-1.5 px-3 text-right font-extrabold text-slate-700 border border-gray-200">₹43,920</td>
                  <td className="py-1.5 px-3 text-right font-black text-[#002fbe] border border-gray-200">₹48,560</td>
                  <td className="py-1.5 px-3 text-right text-green-600 font-bold border border-gray-200">₹48,560</td>
                </tr>
                <tr className="h-[36px] bg-slate-50/30">
                  <td className="py-1.5 px-3 font-black text-orange-700 border border-gray-200">D Wing</td>
                  <td className="py-1.5 px-3 text-red-600 font-bold border border-gray-200">Lotus Block</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">G + 6</td>
                  <td className="py-1.5 px-3 text-center border border-gray-200">14</td>
                  <td className="py-1.5 px-3 text-right border border-gray-200">8,400 sqft</td>
                  <td className="py-1.5 px-3 text-right font-black text-blue-700 border border-gray-200">₹2.88L</td>
                  <td className="py-1.5 px-3 text-right font-extrabold text-slate-700 border border-gray-200">₹40,220</td>
                  <td className="py-1.5 px-3 text-right font-black text-[#002fbe] border border-gray-200">₹42,390</td>
                  <td className="py-1.5 px-3 text-right text-green-600 font-bold border border-gray-200">₹42,390</td>
                </tr>

                {/* TOTAL Summary Row */}
                <tr className="h-[38px] bg-sky-100/90 font-black border-t-2 border-sky-300 text-sky-950 align-middle [&>td]:align-middle [&>td]:py-0">
                  <td colSpan={2} className="py-2 px-3 uppercase text-[9.5px] tracking-wider select-none font-black text-sky-950 border border-sky-300">TOTAL ALL WINGS (4 WINGS)</td>
                  <td className="py-2 px-3 text-center font-extrabold whitespace-nowrap border border-sky-300">G + 27</td>
                  <td className="py-2 px-3 text-center font-black whitespace-nowrap text-[#002fbe] border border-sky-300">67 Units</td>
                  <td className="py-2 px-3 text-right font-black whitespace-nowrap border border-sky-300">42,626 sqft</td>
                  <td className="py-2 px-3 text-right font-black whitespace-nowrap text-[#002fbe] border border-sky-300">₹14.61L</td>
                  <td className="py-2 px-3 text-right font-black whitespace-nowrap text-slate-800 border border-sky-300">₹1,72,060</td>
                  <td className="py-2 px-3 text-right font-black whitespace-nowrap text-[#002fbe] border border-sky-300">₹2,24,040</td>
                  <td className="py-2 px-3 text-right font-black whitespace-nowrap text-emerald-700 border border-sky-300">₹2,24,040</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div ref={containerRef} className="w-full flex flex-col overflow-hidden relative" style={{ minHeight: 0 }}>
          <div className="w-full flex overflow-hidden relative" style={{ minHeight: 0 }}>
            ...
            {!minimizedPanels.middle && (
              <div className="flex flex-col bg-[#f5f8fc] overflow-hidden transition-all duration-300" style={{ width: `${panelWidths.middle}%` }}>
                <div className="bg-[#fdf8e2] border-b border-amber-255/60 px-3 py-1.5 flex items-center justify-between h-[34px] shrink-0 relative">
          ...
          {/* Bottom Restore Bar - renders neatly at bottom when any panel is minimized without obscuring headers */}
          {(minimizedPanels.left || minimizedPanels.middle || minimizedPanels.right) && (
            <div className="bg-slate-100/80 border-t border-gray-200 px-3 py-1 flex items-center justify-between text-[9px] font-bold text-slate-600 shrink-0 select-none animate-fadeIn">
              <div className="flex items-center gap-2">
                <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[8.5px]">Hidden Panels:</span>
                {minimizedPanels.left && <span className="bg-emerald-100 text-emerald-800 px-1.5 py-0.25 rounded text-[8px] font-extrabold">Existing Assessment</span>}
                {minimizedPanels.middle && <span className="bg-amber-100 text-amber-800 px-1.5 py-0.25 rounded text-[8px] font-extrabold">Difference Engine</span>}
                {minimizedPanels.right && <span className="bg-blue-100 text-blue-800 px-1.5 py-0.25 rounded text-[8px] font-extrabold">New Survey</span>}
              </div>
              <button
                onClick={restoreAllPanels}
                className="flex items-center gap-1 bg-white hover:bg-slate-50 border border-slate-300 text-[#002fbe] px-2 py-0.5 rounded-full font-extrabold text-[8.5px] shadow-2xs cursor-pointer transition-all"
                title="Restore all 3 comparison tables"
              >
                <RotateCcw size={10} /> Restore All Tables
              </button>
            </div>
          )}
        </div>
