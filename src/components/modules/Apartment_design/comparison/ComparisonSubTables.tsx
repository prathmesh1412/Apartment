...
export function PreviousAssessmentTable({ rows, onOpenRules, getStatusBgClass, getStatusBadgeClass, scrollContainerRef, isAllWingsSelected }: SubTableProps) {
  ...
  return (
    <div ref={scrollContainerRef} className={`w-full scrollbar-thin overflow-y-hidden ${isAllWingsSelected ? "overflow-auto h-[400px]" : "overflow-x-auto"}`}>
      <table className="w-max text-left border-collapse text-[10px]">
        ...
        <tbody className="divide-y divide-gray-150">
          {rows.map((row, index) => {
            ...
            return (
              <tr key={index} className={`h-[36px] text-gray-700 hover:bg-gray-50/50 align-middle [&>td]:align-middle [&>td]:py-0 ${bgClass}`}>
          ...
          <tr className="h-[36px] bg-sky-100/80 font-black border-t border-sky-300 text-sky-900 align-middle [&>td]:align-middle [&>td]:py-0">
            ...
            <td colSpan={12} className="whitespace-nowrap border-l border-sky-200/50"></td>
          </tr>
        </tbody>
      </table>
    </div>
      ...
      <table className="w-max text-left border-collapse text-[10px]">
          ...
          <tr className="bg-[#edf2ff]/45 border-b border-gray-250 text-[#1e40af] font-black uppercase h-[32px] align-middle [&>th]:align-middle [&>th]:py-0">
            <th className="py-2 px-3 text-center whitespace-nowrap">
              <div className="flex items-center justify-center gap-1">
                <Settings size={11} className="text-[#1e40af]" />
                <span>Action</span>
              </div>
            </th>
            <th className="py-2 px-3 whitespace-nowrap">Prop</th>
        ...
        <tbody className="divide-y divide-gray-150">
          {rows.map((row, index) => {
            ...
            return (
              <tr key={index} className={`h-[36px] text-gray-700 hover:bg-gray-50/50 align-middle [&>td]:align-middle [&>td]:py-0 ${bgClass}`}>
                <td className="py-1 px-2.5 text-center font-extrabold text-gray-400 whitespace-nowrap">
                  <div className="flex items-center justify-center gap-1">
                    <span>{index + 1}</span>
                    <button
                      type="button"
                      className="w-5 h-5 rounded border border-blue-300 bg-blue-50/70 hover:bg-blue-100 text-blue-600 flex items-center justify-center cursor-pointer transition-colors shadow-2xs group"
                      title={`Edit record #${index + 1}`}
                      onClick={(e) => {
                        e.stopPropagation();
                      }}
                    >
                      <Pencil size={9.5} className="group-hover:scale-110 transition-transform" />
                    </button>
                  </div>
                </td>
                ...
                <td className="py-1 px-3 font-semibold text-gray-555 whitespace-nowrap">{row.currUse}</td>
                ...
                <td className="py-1 px-3 text-center font-bold text-gray-555 whitespace-nowrap">{row.rvCvm}</td>
