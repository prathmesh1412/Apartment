      ...
      <div className="bg-white border border-gray-150 rounded-xl p-5 shadow-sm mb-6">
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="col-span-1 md:col-span-2">
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Property Details (UPIC / Owner / Mobile)</span>
            <div className="relative flex items-center border border-gray-200 rounded-md overflow-hidden bg-white shadow-inner h-8">
              <Input 
                naked
                type="text" 
                value={searchQuery}
                ...
                className="w-full py-1.5 px-3 text-xs outline-none placeholder-gray-400 bg-transparent border-0"
              />
              <span className="p-2 text-gray-400"><Search size={15} /></span>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Ward / Division</span>
            <Select 
              placeholder="All Divisions"
              options={[
                { label: 'All Divisions', value: 'All Divisions' },
                { label: 'घोडबंदर मानपाडा', value: 'घोडबंदर मानपाडा' },
                { label: 'कोपरी', value: 'कोपरी' },
                { label: 'वागळे इस्टेट', value: 'वागळे इस्टेट' },
              ]}
              selectSize="sm"
              className="text-xs bg-white h-8 border-gray-200"
            />
          </div>
          <div className="flex gap-2">
            <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-2 rounded-md transition-colors shadow-sm flex items-center justify-center gap-1.5 cursor-pointer h-8">
              <Search size={14} /> Search
            </button>
            <button type="button" onClick={() => { setSearchQuery(''); setResults([]); setSearched(false); }} className="px-3 py-2 border border-gray-200 hover:bg-gray-50 rounded-md text-gray-500 transition-colors flex items-center justify-center cursor-pointer h-8">
              <RefreshCw size={14} />
            </button>
          </div>
        </form>
